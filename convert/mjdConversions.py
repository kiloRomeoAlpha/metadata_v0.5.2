...
#
#                                             metaData.convert.mjdConversions.py
#                                                      Kenneth Anderson, 2011-11
#                                                            ken.anderson@ubc.ca
# ------------------------------------------------------------------------------
...
# Note: Grabbed from Prasanth libs @ Comfort at 1 AU
# http://oneau.wordpress.com/2010/06/20/julian-date-calculator/
# Appears robust. CLI demonstrates commutation.
#
# Eg. real world test, from CASA_h121all.3mm.image
#
# mjd  = 49558.320659722245           # a Casa Image 'obsdate'
# Date = '1994-07-25T07:41:45.000002' # FITS image representation
#
# mjdConversion.caldate(mjd)
# (1994L, 7L, 25L, 7, 41, 45.000001983717084)  # Bang on.
# 
# As noted below, return value from caldate has been modified to
# return an ISO 8601 compliant date-time string of the form
...
def caldate(mjd):
  ...
  ###############    Modified   #############
  # Modified to return an ISO 8601 compliant date format string.
  # k.r. anderson, 2011-08-22T17:14:00 - 7H GMT.

  dateString = ''
  for sect in year, month, day:
    stringSect = str(sect)
    if sect < 10:
      dateString += "0"+stringSect+"-"
      continue
    dateString += stringSect+"-"
  dateString = dateString[:-1]
  dateString += "T"
  for sect in int(sign+str(hour)), minute, second:
    stringSect = str(sect)
    if sect < 10:
      dateString += "0"+stringSect+":"
      continue
